    ...
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-white antialiased selection:bg-indigo-600 selection:text-white">
      ...
      <main className="pt-32 pb-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-16">
        ...
        <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}>
          <Link href="/products" className="inline-flex items-center gap-2 text-sm font-black uppercase tracking-widest text-zinc-400 hover:text-indigo-600 transition-colors group">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          ...
          <div className="lg:col-span-7 bg-white dark:bg-zinc-900 border border-zinc-200/60 dark:border-zinc-800 p-6 sm:p-8 rounded-3xl space-y-6 shadow-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm font-bold text-zinc-600 dark:text-zinc-400 border-t border-zinc-100 dark:border-zinc-800 pt-5">
              {displayProduct.features.map((feature, i) => (
                <div key={i} className="flex items-center gap-2.5">
                  <CheckCircle2 className="w-4.5 h-4.5 text-emerald-500 shrink-0" /> <span>{feature}</span>
                </div>
              ))}
            </div>
            ...
            <div className="p-5 bg-zinc-50 dark:bg-zinc-950/50 rounded-2xl border border-zinc-100 dark:border-zinc-800 grid grid-cols-1 sm:grid-cols-2 gap-5">
              <div className="space-y-3">
                <label className="block text-xs font-black uppercase tracking-widest text-zinc-400">Select Volume Tier</label>
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-2">
              <Link 
                ...
                className="flex-1 bg-indigo-600 border border-indigo-500 text-white text-sm font-black uppercase tracking-widest text-center py-2.5 sm:py-4 rounded-xl hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-600/10 flex items-center justify-center gap-1.5"
              >
                Get Custom Quote
              </Link>
              <Link 
                href={`/samples?target=${slug}`} 
                className="flex-1 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 text-zinc-700 dark:text-zinc-200 text-sm font-black uppercase tracking-widest text-center py-2.5 sm:py-4 rounded-xl hover:bg-zinc-50 dark:hover:bg-zinc-700/60 transition-all flex items-center justify-center gap-1.5"
              >
                Request Samples
              </Link>
            </div>
        ...
        <div className="space-y-6">
          <div className="flex border-b border-zinc-200/60 dark:border-zinc-800 gap-6 overflow-x-auto">
            {(["specs", "pricing", "categories"] as const).map((tab) => (
              <button
                key={tab} onClick={() => setActiveTab(tab)}
                className={`pb-3 text-sm font-black uppercase tracking-widest border-b-2 transition-all shrink-0 ${
            ...
            {[
              ...
              <div key={related.id} className="bg-white dark:bg-zinc-900 border border-zinc-200/60 dark:border-zinc-800 p-6 rounded-2xl flex flex-col justify-between space-y-4 shadow-sm hover:shadow-xl transition-all duration-300">
                ...
                <div className="flex items-center justify-between border-t border-zinc-100 dark:border-zinc-800 pt-4">
                  ...
                  <Link href={`/products/${related.id}`} className="text-xs font-black uppercase tracking-widest text-zinc-400 flex items-center gap-1 hover:text-indigo-500 transition-colors">
        ...
        <section className="bg-zinc-900 text-white p-8 sm:p-12 rounded-3xl border border-zinc-800 shadow-xl relative overflow-hidden flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex gap-4 shrink-0 relative z-10">
            <Link 
              href={`/quote?product=${slug}`} 
              className="bg-indigo-600 border border-indigo-500 text-white text-sm font-black uppercase tracking-widest px-6 py-2.5 sm:py-4 rounded-xl hover:bg-indigo-700 transition-all shadow-md shadow-indigo-600/20"
            >
              Get Custom Quote
            </Link>
            <Link 
              href="/contact" 
              className="bg-zinc-950 border border-zinc-800 text-zinc-300 text-sm font-black uppercase tracking-widest px-6 py-2.5 sm:py-4 rounded-xl hover:bg-zinc-800 transition-all"
            >
              Contact Us
            </Link>
          </div>
