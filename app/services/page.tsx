    ...
    <main className="min-h-screen bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-white pt-20 overflow-x-hidden antialiased">
      ...
      <section className="relative overflow-hidden border-b border-zinc-200/60 dark:border-zinc-900 bg-white dark:bg-zinc-950 pt-32 pb-24">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-indigo-50 dark:bg-indigo-950/40 text-indigo-600 dark:text-indigo-400 text-xs font-bold uppercase tracking-wider mb-6">
            <Sparkles className="w-3.5 h-3.5" /> Production Capabilities Center
          </div>
          ...
          <p className="mt-6 max-w-3xl text-base sm:text-lg text-zinc-500 dark:text-zinc-400 font-medium leading-relaxed">
            ...
            >
              ...
              <div className="space-y-2 border-b border-zinc-200 dark:border-zinc-800 pb-6">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  ...
                  <span className="self-start sm:self-auto text-[11px] font-mono font-bold uppercase tracking-widest text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950/40 border border-indigo-100 dark:border-indigo-900/40 px-3 py-1 rounded-lg">
                    {cat.items.length} Active Production Lines
                  </span>
                </div>
                <p className="text-xs sm:text-sm text-zinc-400 font-medium max-w-3xl leading-relaxed">{cat.description}</p>
              </div>
                  ...
                  >
                    <div className="space-y-4">
                      <div className="space-y-1.5">
                        <h3 className="font-black text-lg text-zinc-900 dark:text-white group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors tracking-tight">
                          {item.name}
                        </h3>
                      ...
                      <div className="bg-zinc-50 dark:bg-zinc-950 p-4 rounded-xl border border-zinc-100 dark:border-zinc-900 grid grid-cols-2 gap-y-3 gap-x-2 text-[11px]">
                        <div>
                          <span className="block text-zinc-400 font-bold uppercase tracking-wider scale-95 origin-left">Operational Velocity</span>
                          <span className="font-mono font-bold text-zinc-800 dark:text-zinc-200">{item.specs.speed}</span>
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <span className="font-mono font-bold text-indigo-600 dark:text-indigo-400">{item.specs.tolerance}</span>
                        </div>
                      </div>
                    </div>

                    <div className="pt-6 border-t border-zinc-100 dark:border-zinc-950 mt-4 flex items-center justify-between">
                      <Link
                        href={`/services/${item.id}`}
                        className="inline-flex items-center gap-1.5 text-xs font-black text-indigo-600 dark:text-indigo-400 group-hover:translate-x-1 transition-transform"
                      >
                        Technical Requirements Matrix <ArrowRight className="w-3.5 h-3.5" />
                      </Link>
                    </div>
                  </motion.div>
                ))}
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pb-12">
        <div className="bg-zinc-900 border border-zinc-800 p-8 sm:p-12 rounded-3xl text-white grid grid-cols-1 lg:grid-cols-3 gap-12 items-center relative overflow-hidden">
          ...
          <div className="lg:col-span-1 space-y-3">
            <div className="w-10 h-10 rounded-xl bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 flex items-center justify-center"><ShieldAlert className="w-5 h-5" /></div>
            <h3 className="text-2xl font-black tracking-tight pt-2">Prepress Validation Requirements</h3>
            <p className="text-xs text-zinc-400 leading-relaxed font-medium">To maintain our high operational efficiency, ensure all incoming digital graphic layouts strictly follow our shop floor file profile system parameters.</p>
          </div>

          <div className="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-6 text-xs font-medium">
            <div className="bg-zinc-950/60 p-5 rounded-2xl border border-zinc-800 space-y-1.5">
              <h4 className="font-bold text-zinc-200 flex items-center gap-2"><FileCheck className="w-4 h-4 text-indigo-400" /> Native Color Space Profiles</h4>
            ...
            <div className="bg-zinc-950/60 p-5 rounded-2xl border border-zinc-800 space-y-1.5">
              <h4 className="font-bold text-zinc-200 flex items-center gap-2"><Gauge className="w-4 h-4 text-indigo-400" /> Bleed & Safe Trim Margins</h4>
            ...
            <div className="bg-zinc-950/60 p-5 rounded-2xl border border-zinc-800 space-y-1.5">
              <h4 className="font-bold text-zinc-200 flex items-center gap-2"><Minimize2 className="w-4 h-4 text-indigo-400" /> Vector Paths & Font Preservation</h4>
            ...
            <div className="bg-zinc-950/60 p-5 rounded-2xl border border-zinc-800 space-y-1.5">
              <h4 className="font-bold text-zinc-200 flex items-center gap-2"><Eye className="w-4 h-4 text-indigo-400" /> Image Resolution Matrix</h4>
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pb-24 pt-12">
        <div className="relative overflow-hidden bg-gradient-to-b from-indigo-600 to-indigo-700 text-white p-12 rounded-3xl shadow-xl shadow-indigo-600/10 text-center">
          ...
          <div className="pt-6">
            <Link
              href="/contact"
              className="inline-flex items-center gap-2 bg-zinc-950 text-white font-black px-8 py-4 rounded-xl shadow-lg hover:bg-zinc-900 transition-colors text-sm"
            >
