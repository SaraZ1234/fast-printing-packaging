...
const cardItemVariants = {
  ...
  show: { opacity: 1, y: 0, transition: { type: "spring", stiffness: 100, dampening: 15 } }
};
    ...
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-white antialiased selection:bg-indigo-600 selection:text-white">
      ...
      <main className="pt-36 pb-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-24">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-4">
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-indigo-50 dark:bg-indigo-950/40 text-indigo-600 dark:text-indigo-400 text-xs font-black tracking-widest uppercase">
            Market Integration Matrix
          </span>
          <h1 className="text-4xl sm:text-6xl font-black tracking-tight text-zinc-900 dark:text-white">
            Tailored Industry Solutions
          </h1>
          <p className="text-sm sm:text-base text-zinc-500 dark:text-zinc-400 font-medium max-w-2xl mx-auto leading-relaxed">
          ...
          <div className="pt-6 flex flex-wrap justify-center gap-1.5 max-w-2xl mx-auto">
            ...
            {filteredSectors.map((item) => {
              ...
              return (
                <motion.div
                  ...
                  layout
                  whileHover={{ y: -5 }}
                  className="group bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-zinc-200/60 dark:border-zinc-800 flex flex-col justify-between shadow-sm hover:shadow-xl hover:border-indigo-500/30 transition-all duration-300 relative overflow-hidden"
                >
                  ...
                  <div className="space-y-5">
                    <div className="flex items-center justify-between">
                      <div className="w-12 h-12 rounded-2xl bg-indigo-50 dark:bg-indigo-950/40 flex items-center justify-center text-indigo-600 dark:text-indigo-400 group-hover:bg-indigo-600 group-hover:text-white transition-colors duration-300 shadow-inner">
                        <IconComponent className="w-5 h-5" />
                      </div>
                      <span className="text-[9px] font-mono font-bold tracking-widest bg-zinc-100 dark:bg-zinc-800 text-zinc-500 px-2 py-0.5 rounded uppercase">
                        {item.tier}
                      </span>
                    </div>

                    <div className="space-y-2">
                      <h3 className="text-lg font-black text-zinc-900 dark:text-white group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors tracking-tight">
                        {item.name}
                      </h3>
                  ...
                  <div className="pt-6 border-t border-zinc-100 dark:border-zinc-800/60 mt-4 flex items-center justify-between">
                    <Link 
                      ...
                      className="inline-flex items-center gap-1.5 text-xs font-black uppercase tracking-widest text-indigo-600 dark:text-indigo-400 group-hover:text-indigo-700 dark:group-hover:text-indigo-300 transition-colors"
                    >
                      Case Studies
                    </Link>
        ...
        <section className="bg-white dark:bg-zinc-900 border border-zinc-200/60 dark:border-zinc-800 p-8 sm:p-12 rounded-3xl shadow-sm grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 divide-y sm:divide-y-0 lg:divide-x divide-zinc-100 dark:divide-zinc-800">
          {[
            ...
            <div key={i} className={`space-y-1.5 ${i > 0 ? "pt-6 sm:pt-0 lg:pl-8" : ""}`}>
              <span className="block font-mono text-3xl font-black text-indigo-600 dark:text-indigo-400 tracking-tight">{stat.metric}</span>
              <h4 className="font-black text-xs text-zinc-900 dark:text-white uppercase tracking-wider">{stat.title}</h4>
              <p className="text-xs text-zinc-500 dark:text-zinc-400 font-medium leading-relaxed">{stat.desc}</p>
            </div>
          ))}
        </section>
        ...
        <section className="space-y-12">
          <div className="text-center max-w-xl mx-auto space-y-2">
            ...
            <h2 className="text-2xl sm:text-3xl font-black text-zinc-900 dark:text-white tracking-tight">Our Manufacturing Framework</h2>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 relative">
            {workflowSteps.map((step, idx) => {
              ...
              return (
                <div key={idx} className="bg-white dark:bg-zinc-900/40 border border-zinc-200/50 dark:border-zinc-800/80 p-6 rounded-2xl relative space-y-4 shadow-inner">
                  <div className="flex items-center justify-between">
                    <div className="p-2.5 rounded-xl bg-zinc-100 dark:bg-zinc-800 text-zinc-800 dark:text-zinc-200">
                      <StepIcon className="w-5 h-5" />
                    </div>
                    <span className="font-mono text-xs font-black text-zinc-300 dark:text-zinc-700">{step.step}</span>
                  </div>
                  <div className="space-y-1">
                    <h4 className="font-black text-sm text-zinc-900 dark:text-white tracking-tight">{step.title}</h4>
                    <p className="text-xs text-zinc-500 dark:text-zinc-400 font-medium leading-relaxed">{step.desc}</p>
                  </div>
                </div>
              );
            })}
        ...
        <section className="bg-zinc-900 dark:bg-zinc-900 border border-zinc-800 text-white p-8 sm:p-12 rounded-3xl shadow-xl relative overflow-hidden flex flex-col md:flex-row md:items-center justify-between gap-8">
          <div className="absolute top-0 right-0 w-96 h-96 bg-indigo-600/10 rounded-full blur-3xl pointer-events-none" />
          <div className="space-y-3 max-w-xl relative z-10">
            <h2 className="text-2xl sm:text-3xl font-black tracking-tight">Looking for a Dedicated Corporate Printing Framework?</h2>
          ...
          <div className="flex flex-col sm:flex-row gap-3 shrink-0 relative z-10 w-full md:w-auto">
            <Link href="/quote" className="bg-indigo-600 border border-indigo-500 text-white text-xs font-black uppercase tracking-widest px-6 py-4 rounded-xl hover:bg-indigo-700 text-center transition-all shadow-lg shadow-indigo-600/20">
              Initialize Custom Setup
            </Link>
            <Link href="/contact" className="bg-zinc-950 border border-zinc-800 text-zinc-300 text-xs font-black uppercase tracking-widest px-6 py-4 rounded-xl hover:bg-zinc-800 text-center transition-all">
              Consult an Engineer
            </Link>
          </div>
        </section>
